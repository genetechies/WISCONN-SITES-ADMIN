...
public partial class translation_law_quotation : System.Web.UI.Page
{
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        {
            Response.Redirect("translation-law-contact.html");
        }
        //1小時內提交超過10次加入黑名單
        if (CheckIP(ip) >= 10)
        {
        ...
        try
        {
            ...
            using (SqlConnection conn = new SqlConnection(PubConstant.ConnectionString))
            {
                if (conn.State != ConnectionState.Open)
                    conn.Open();
                string sql = "insert into  " + Mcon + ".dbo.XunJia(LinksName,LinksTel,LinksEmail,SerPro,OrgLanguage,ToLanguage,TxtCount,TxtSCount,ispaiban,ercijiaogao,rungao,JiaojianTime,gongzuori,zhuyicontent,Annex,addTime,ip,Finish,[Note]) values(@LinksName,@LinksTel,@LinksEmail,@SerPro,@OrgLanguage,@ToLanguage,@TxtCount,@TxtSCount,@ispaiban,@ercijiaogao,@rungao,@JiaojianTime,@gongzuori,@zhuyicontent,@Annex,@addTime,@ip,@Finish,@Note) select @@identity";                //string sql = "insert into XunJia(LinksName,LinksTel,LinksEmail,SerPro,OrgLanguage,ToLanguage,TxtCount,TxtSCount,ispaiban,ercijiaogao,rungao,JiaojianTime,gongzuori,zhuyicontent,Annex,addTime) values(@LinksName,@LinksTel,@LinksEmail,@SerPro,@OrgLanguage,@ToLanguage,@TxtCount,@TxtSCount,@ispaiban,@ercijiaogao,@rungao,@JiaojianTime,@gongzuori,@zhuyicontent,@Annex,@addTime)";
                ...
                cmd.Parameters.AddWithValue("@Note", "來自法律站點");
            ...
            MyTool.alert("感謝您的詢價，我們已收到，會儘速請報價專員與您聯絡！", "translation-law-quotation.aspx");
