...
public partial class translation_law_information : System.Web.UI.Page
{
    BLL.NewsClass newClass = new NewsClass();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            if (Request.QueryString["cid"] != null)
            {
                ...
                Model.NewsClass newClass = new BLL.NewsClass().GetModel(Int32.Parse(Request.QueryString["cid"]));
                Title = "翻譯資訊-"+newClass.ClassName+ "-后冠法律翻譯社";

                ltArtListTitle.Text = newClass.ArtListTitle;
                ltArtListDescription.Text = newClass.ArtListDescription;
            }
            else
            {
                Where = "D_Recycle=0 ";
