 ...
 * @param {Function} iterator 对每個数组元素进行调用的函数，该函数有两個参数，第一個为数组元素，第二個为数组索引值，function (item, index)。
...
/**
 * opera 从10開始不是用opera后面的字符串进行版本的判断
 ...
 * 
 * @param {Object} 	options 					创建flash的選项参数
 ...
 * @param {string} 	options.base 				设置用于解析swf檔案中的所有相对路径语句的基本目录或URL
 * @param {string} 	options.bgcolor 			swf檔案的背景色
 * @param {string} 	options.salign 				设置缩放的swf檔案在由width和height设置定义的区域内的位置。允许值：l/r/t/b/tl/tr/bl/br
 * @param {boolean} options.menu 				是否显示右键菜单，允许值：true/false
 * @param {boolean} options.loop 				播放到最后一帧时是否重新播放，允许值： true/false
 * @param {boolean} options.play 				flash是否在浏览器加载时就開始播放。允许值：true/false
 ...
 * @config {Number} vars.gridWidth 每一個预览圖片所占的宽度，应该为flash寛的整除
 * @config {Number} vars.gridHeight 每一個预览圖片所占的高度，应该为flash高的整除
 * @config {Number} vars.picWidth 单張预览圖片的宽度
 * @config {Number} vars.picHeight 单張预览圖片的高度
 * @config {String} vars.uploadDataFieldName POST请求中圖片数据的key,默认值'picdata'
 * @config {String} vars.picDescFieldName POST请求中圖片描述的key,默认值'picDesc'
 * @config {Number} vars.maxSize 檔案的最大体积,单位'MB'
 * @config {Number} vars.compressSize 上傳前如果圖片体积超过该值，会先压缩
 ...
 * @param {String|Function} selectFileCallback 選擇檔案的回调
 * @param {String|Function} exceedFileCallback檔案超出限制的最大体积时的回调
 * @param {String|Function} deleteFileCallback 删除檔案的回调
 * @param {String|Function} startUploadCallback 開始上傳某個檔案时的回调
 * @param {String|Function} uploadCompleteCallback 某個檔案上傳完成的回调
 * @param {String|Function} uploadErrorCallback 某個檔案上傳失败的回调
 * @param {String|Function} allCompleteCallback 全部上傳完成时的回调
...
baidu.flash.fileUploader = baidu.flash.fileUploader || function(options){
     ...
     * @param {Object} postData 键值对，上傳的POST数据
 ...
 * @config {Integer} [timeOut] 超时时间，超过这個时间将不再响应本请求，并触发onfailure函数
 * @config {Function} [onfailure] timeOut设定后才生效，到达超时时间时触发本函数
 * @remark
 * 1、与callByServer不同，callback参数只支持Function类型，不支持string。
 * 2、如果请求了一個不存在的页面，callback函数在IE/opera下也会被调用，因此使用者需要在onsuccess函数中判断数据是否正确加载。
 * @meta standard
 ...
 * @param {Object} opt_options 加载数据时的選项.
 * @config {string} [charset] script的字符集
 * @config {string} [queryField] 服务器端callback请求字段名，默认为callback
 * @config {Integer} [timeOut] 超时时间(单位：ms)，超过这個时间将不再响应本请求，并触发onfailure函数
 * @config {Function} [onfailure] timeOut设定后才生效，到达超时时间时触发本函数
 * @remark
...
baidu.sio.callByServer = /**@function*/function(url, callback, opt_options) {
    ...
    /*
     * 返回一個函数，用于立即（挂在window上）或者超时（挂在setTimeout中）时执行
     */
 ...
 * @remark
 * 该方法的实现与ecma-262第五版中规定的JSON.parse不同，暂时只支持传入一個参数。后续会进行功能丰富。
 * @meta standard
 ...
 * @remark
 * 该方法的实现与ecma-262第五版中规定的JSON.stringify不同，暂时只支持传入一個参数。后续会进行功能丰富。
 * @meta standard
