...
namespace DAL
{
    public class NewsClass
    {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ClassId,ClassName,OrderID,(select ClassName from NewsClass t2 where t2.classid=t1.ParentID) as parentclassname FROM NewsClass t1 ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top " + num + " classid,ClassName,OrderID,(select ClassName from NewsClass t2 where t2.classid=t1.ParentID) as parentclassname FROM NewsClass t1  ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
