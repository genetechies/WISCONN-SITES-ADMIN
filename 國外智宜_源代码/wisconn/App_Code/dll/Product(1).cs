...
namespace ZeroStudio.DAL
{
    public class Product
    {
        ...
        {
            StringBuilder sql = new StringBuilder();
            sql.Append("select count(1) from product inner join ProductClass on product.P_ClassID=ProductClass.PC_Id");
            if (!string.IsNullOrEmpty(where))
            {
                sql.Append(" where " + where);
            }
        ...
        public void Add(ZeroStudio.Model.Product model)
        {
            using (OleDbConnection conn = new OleDbConnection(PubConstant.ConnectionString))
            {
                if (conn.State != ConnectionState.Open)
                    conn.Open();
                string sql = "insert into Product(P_Name,P_Name_En,P_Code,P_Model,P_Spec,P_Stock,P_Identity,P_Evinr,P_ClassID,P_ParentID,P_PhotoUrl,P_Doc,P_Uptime,P_State,Description,Keywords,UpKey,DownKey) " +
                    "values(@P_Name,@P_Name_En,@P_Code,@P_Model,@P_Spec,@P_Stock,@P_Identity,@P_Evinr,@P_ClassID,@P_ParentID,@P_PhotoUrl,@P_Doc,@P_Uptime,@P_State,@description,@keywords,@UpKey,@DownKey)";
        ...
        public void Update(ZeroStudio.Model.Product model)
        {
            using (OleDbConnection conn = new OleDbConnection(PubConstant.ConnectionString))
            {
                if (conn.State != ConnectionState.Open)
                    conn.Open();
                string sql = "update Product set P_Name=@P_Name,P_Name_En=@P_Name_En,P_Code=@P_Code,P_Model=@P_Model,P_Spec=@P_Spec,P_Stock=@P_Stock,P_Identity=@P_Identity,P_Evinr=@P_Evinr,P_ClassID=@P_ClassID,P_ParentID=@P_ParentID,P_PhotoUrl=@P_PhotoUrl,P_Doc=@P_Doc,P_State=@P_State,Keywords=@keywords,Description=@description,UpKey=@UpKey,DownKey=@DownKey where P_ID=@P_ID";
                OleDbCommand cmd = new OleDbCommand(sql, conn);
                cmd.Parameters.AddWithValue("@P_Name", model.P_Name);
                cmd.Parameters.AddWithValue("@P_Name_En", model.P_Name_En);
                cmd.Parameters.AddWithValue("@P_Code", model.P_Code);
        ...
        public void UpdaetSortKey(int P_Id, int sortkey)
        {
            ...
            {
                DbHelperOledb.ExecuteSql("update Product set P_ParentID=P_ParentID+1 where  P_ParentID >="+ sortkey + " and P_ParentID<=" + model.P_ParentID + " and P_ClassId=" + model.P_ClassID);
            }
            else //输值增大
            {
                DbHelperOledb.ExecuteSql("update Product set P_ParentID=P_ParentID-1 where  P_ParentID <=" + sortkey + " and P_ParentID>=" + model.P_ParentID + " and P_ClassId=" + model.P_ClassID);
        ...
        public ZeroStudio.Model.Product GetModel(int P_ID)
        {
            ...
            using (OleDbConnection conn = new OleDbConnection(PubConstant.ConnectionString))
            {
                ...
                if (sdr.Read())
                {
                    ...
                    model.P_ClassID = (int)sdr["P_ClassID"];
        ...
        */
        public DataSet GetListByClass(string strWhere, string orderby)
        {
            ...
            {
                strSql.Append(" order by P_ClassID, P_ParentID desc,PC_Id");
            }
            return DbHelperOledb.Query(strSql.ToString());
        }
