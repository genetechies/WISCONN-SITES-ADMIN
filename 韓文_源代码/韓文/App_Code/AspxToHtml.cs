...
public class AspxToHtml : System.Web.UI.Page
{
    ...
    public void WriteFile(string htmlName)
    {
        ...
        if (htmlName == "downhouguan-korean-index.aspx")
            friendlytable = CreateTable2(" F_location in (1,3,4,6)");
        else if (htmlName == "downother.html")
            friendlytable = CreateTable2(" F_location IN (1,5,6,7)");
        else if (htmlName == "downlink.html")
            friendlytable = CreateTable2(" F_location in (1,2,4,7)");
        else
            friendlytable = CreateTable2("");

        string FriendlyName = "";
        //替换内容
        if (friendlytable != null && friendlytable.Rows.Count >= 5)
        {
            for (int i = 0; i < friendlytable.Rows.Count; i++)
            {
                FriendlyName += "<li><a href='http://" + friendlytable.Rows[i]["F_Url"].ToString() + "' target='_blank' title='" + friendlytable.Rows[i]["F_Name"].ToString() + "'>" + friendlytable.Rows[i]["F_Name"].ToString() + "</a></li>";
            }
        }
        if (friendlytable != null && friendlytable.Rows.Count < 5)
        {            
            for (int i = 0; i < 5 - friendlytable.Rows.Count; i++)
            {
                FriendlyName += "<li><a href='http://" + friendlytable.Rows[i]["F_Url"].ToString() + "' target='_blank' title='" + friendlytable.Rows[i]["F_Name"].ToString() + "'>" + friendlytable.Rows[i]["F_Name"].ToString() + "</a></li>";
            }            
        }
    ...
    public DataTable CreateTable(string where)
    {
        ...
        DataSet ds = DBUtility.DbHelperSQL.Query(string.Format("select top 5 * from Friendly {0}  order by F_SortKey asc,F_Id desc", where));
        ...
        DataSet ds2 = DBUtility.DbHelperSQL.Query(string.Format("select top 5 * from Partners {0}  order by F_SortKey asc,F_Id desc", where));
