...
public class AspxToHtml : System.Web.UI.Page
{
    ...
    public DataTable CreateTable(string where)
    {
        ...
        DataSet ds = DBUtility.DbHelperSQL.Query(string.Format("select top 16 * from Friendly {0}  order by F_SortKey asc,F_Id desc", where));
    ...
    public DataTable CreateTable2(string where)
    {
        ...
        DataSet ds = DBUtility.DbHelperSQL.Query(string.Format("select top 16 * from Partners {0}  order by F_SortKey asc", where));
        for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
        {
            dr = dt.NewRow();
            dr["F_Id"] = Convert.ToInt32(ds.Tables[0].Rows[i]["F_Id"].ToString());
