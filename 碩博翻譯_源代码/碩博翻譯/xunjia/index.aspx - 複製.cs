...
public partial class xunjia_index : System.Web.UI.Page
{
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string posttime = posttimeY.SelectedItem.Value.Trim() + "-" + posttimeM.SelectedItem.Value.Trim() + "-" + posttimeD.SelectedItem.Value.Trim();

            if (!MyTool.IsValidDate(posttime))
            {
            ...
            mo.ercijiaogao = 0;
            mo.rungao = 0;
            mo.JiaojianTime = Convert.ToDateTime(posttime);
            mo.gongzuori = "0";
            ...
            if (file1.HasFile)
            {
                string path = Server.MapPath("/online/UpLoadFiles/") + file1.FileName.Replace(" ", "");
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    title = "管理員" + ds.Tables[0].Rows[i][1].ToString() + "你好,碩博翻譯社網站上發在起線上詢價，請注意查看！";
                    tobody = "管理員" + ds.Tables[0].Rows[i][1].ToString() + "你好,碩博翻譯社網站上發在起線上詢價，請注意查看！" +
                        ...
                        "<br>總字數:" + Convert.ToInt32(translationamount.Value) + ddltranslationtype.SelectedItem.Text + "<br>交件時間:" + Convert.ToDateTime(posttime) +
                        "<br>備註:" + ttbNotes.Text + "<br>此為簡訊,正確資訊請至碩博翻譯社後台查詢!!";
                    //touser = "522125549@qq.com";
                    SendEmail(touser, title, tobody);
                }
            }

            MyTool.alert("感謝您的詢價，我們已收到，會儘速請報價專員與您聯絡！", "/xunjia/index.aspx");
            MyTool.alert("感謝您的詢價，我們已收到，會儘速請報價專員與您聯絡！", "/xunjia/index.aspx");
