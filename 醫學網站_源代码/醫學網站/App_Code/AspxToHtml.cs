...
public class AspxToHtml : System.Web.UI.Page
{
    ...
    public void WriteFile(string htmlName)
    {
        ...
        if (friendlytable != null && friendlytable.Rows.Count >= 7)
        {
            ...
            {
                FriendlyName += "<a href='http://" + friendlytable.Rows[i]["F_Url"].ToString() + "' target='_blank' title='" + friendlytable.Rows[i]["F_Name"].ToString() + "'>" + friendlytable.Rows[i]["F_Name"].ToString() + "</a>";
            }
        }else if(friendlytable != null && friendlytable.Rows.Count < 7)
        {
            FriendlyName = "";
            for (int i = 0; i <  friendlytable.Rows.Count; i++)
            {
    ...
    public DataTable CreateTable(string where)
    {
        ...
        DataSet ds = DBUtility.DbHelperSQL.Query(string.Format("select top 21 * from Friendly {0}  order by F_SortKey asc,F_Id desc", where));
