...
public class AspxToHtml : System.Web.UI.Page
{
    ...
    public void WriteFile(string htmlName)
    {
        ...
        sb.Append(" <section class=\"secure-section\" style=\"height: 320px;\">");
        sb.Append(" <div class=\"container\" style=\"position: relative; top: -40px;\">");

        for (int i = 0; i < friendlytable.Rows.Count; i++)
        {
            if (i % 5 == 0)
            {
                sb.Append("<ul class=\"linkstyle\" >");

            }

            sb.Append("<li >");
            sb.Append("<a href='http://" + friendlytable.Rows[i]["F_Url"].ToString() + "' target='_blank' title='" + friendlytable.Rows[i]["F_Name"].ToString() + "'>" + friendlytable.Rows[i]["F_Name"].ToString() + "</a>");
            sb.Append("</li>");

            if (i % 5 == 4 || i == friendlytable.Rows.Count - 1)
            {
                sb.Append("</ul>");
            }
        }

        sb.Append("<h5>&nbsp;</h5></div></section>");
