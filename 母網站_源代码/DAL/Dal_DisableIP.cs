...
namespace DAL
{
    ...
    public class Dal_DisableIP
    {
        ...
        public bool Exists(int D_Id)
        {
            ...
            strSql.Append("select count(1) from DisableIP");
        ...
        public int Count()
        {
            ...
            sql.Append("select count(1) from DisableIP");
        ...
        public void Add(Model_DisableIP model)
        {
            using (SqlConnection conn = new SqlConnection(PubConstant.ConnectionString))
            {
                if (conn.State != ConnectionState.Open)
                    conn.Open();
                string sql = "insert into DisableIP(IPAddress,AddDate) values(@IPAddress,@AddDate)";
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from DisableIP ");
            strSql.Append(" where D_Id=" + D_Id + " ");
        ...
        public Model.Model_DisableIP GetModel(int D_Id)
        {
            ...
            using (SqlConnection conn = new SqlConnection(PubConstant.ConnectionString))
            {
                if (conn.State != ConnectionState.Open)
                    conn.Open();
                string sql = "select * from DisableIP where id=@D_Id";
                SqlCommand cmd = new SqlCommand(sql, conn);
