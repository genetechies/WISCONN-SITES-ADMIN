...
public class AspxToHtml : System.Web.UI.Page
{
    ...
    public void WriteFile(string htmlName)
    {
        ...
        if (friendlytable != null && friendlytable.Rows.Count > 0)
        {
            ...
            {
                if (i != 0 && i % 7 == 0)
                    FriendlyName += "</tr><tr><td style='border:0px;' algin='center'><div class='icon1'><a href='http://" + friendlytable.Rows[i]["F_Url"].ToString() + "' target='_blank' title='" + friendlytable.Rows[i]["F_Name"].ToString() + "'><font style='font-family: 'Verdana', 'Helvetica san-serif''>" + friendlytable.Rows[i]["F_Name"].ToString() + "</font></a></div></td> ";
                else
                    FriendlyName += "<td  style='border:0px;width:135px;' algin='center'><div class='icon1'><a href='http://" + friendlytable.Rows[i]["F_Url"].ToString() + "' target='_blank' title='" + friendlytable.Rows[i]["F_Name"].ToString() + "'><font style='font-family: 'Verdana', 'Helvetica san-serif''>" + friendlytable.Rows[i]["F_Name"].ToString() + "</font></a></div></td> ";
            }
        }
        ...
        if (htmlName == "downindex.html")
            partnerstable = CreateTable(" F_location in (1,3,4,6)");
        else if (htmlName == "downother.html")
            partnerstable = CreateTable(" F_location in (1,5,6,7)");
        else if (htmlName == "downlink.html")
            partnerstable = CreateTable(" F_location in (1,2,4,7)");
        else
            partnerstable = CreateTable("");
        ...
        if (partnerstable != null && partnerstable.Rows.Count > 0)
        {
            ...
            {
                if (i != 0 && i % 7 == 0)
                    PartnersName += "</tr><tr><td  style='border:0px;' algin='center'><div class='icon1'><a href='http://" + partnerstable.Rows[i]["F_Url"].ToString() + "' target='_blank' title='" + partnerstable.Rows[i]["F_Name"].ToString() + "'><font style='font-family: 'Verdana', 'Helvetica san-serif''>" + partnerstable.Rows[i]["F_Name"].ToString() + "</font></a></div></td> ";
                else
                    PartnersName += "<td algin='center' style='border:0px;width:135px;'><div class='icon1'><a href='http://" + partnerstable.Rows[i]["F_Url"].ToString() + "' target='_blank' title='" + partnerstable.Rows[i]["F_Name"].ToString() + "'><font style='font-family: 'Verdana', 'Helvetica san-serif''>" + partnerstable.Rows[i]["F_Name"].ToString() + "</font></a></div></td>";
            }
        }
    ...
    public DataTable CreateTable(string where)
    {
        ...
        DataSet ds = DBUtility.DbHelperSQL.Query(string.Format("select * from Friendly {0}  order by F_SortKey asc,F_Id desc", where));
        ...
        DataSet ds2 = DBUtility.DbHelperSQL.Query(string.Format("select * from Partners {0}  order by F_SortKey asc,F_Id desc", where));
    ...
    public DataTable CreateTable2(string where)
    {
        ...
        DataSet ds = DBUtility.DbHelperSQL.Query(string.Format("select * from Partners {0}  order by F_SortKey asc", where));
        for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
        {
            dr = dt.NewRow();
            dr["F_Id"] = Convert.ToInt32(ds.Tables[0].Rows[i]["F_Id"].ToString());
